(() => {
  ...
  input.addEventListener("change", e => {
    ...
    if (!file.type.startsWith("image/")) {
      toastMsg("Selecione uma imagem válida");
      e.target.value = "";
      return;
    }
    ...
    newImage.onload = () => {
      ...
      // O stage estava oculto antes do upload. Portanto, o canvas ainda
      // não possuía dimensões. Agora que ele está visível, dimensionamos
      // o canvas antes de calcular a escala e desenhar a imagem.
      requestAnimationFrame(() => {
        resize();
        center();
        fit();
        toastMsg("Desenho carregado");
      });
    ...
    newImage.onerror = () => {
      URL.revokeObjectURL(url);
      toastMsg("Não foi possível abrir essa imagem");
      e.target.value = "";
    };
